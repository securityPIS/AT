...
import {
  ...
  Users, Handshake, Mail, MessageCircle,
} from 'lucide-react';
...
// --- FITUR UPDATE & KOORDINASI (entri non-task dengan LOG progres) ---
// Urutan tetap dipakai untuk render pilihan di form & filter.
export const UPDATE_CATEGORY_ORDER = ['meeting', 'koordinasi', 'email', 'whatsapp'];
export const UPDATE_STATUS_ORDER = ['baru', 'proses', 'tindak_lanjut', 'selesai'];

// Metadata tampilan per kategori update (label, ikon lucide, warna badge/chip).
// Mengikuti pola getEventTypeMeta di lib/taskUtils.js.
export const UPDATE_CATEGORY_META = {
  meeting:    { label: 'Meeting',    icon: Users,          badgeClass: 'bg-indigo-100 text-indigo-700 border-indigo-200',   chipClass: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
  koordinasi: { label: 'Koordinasi', icon: Handshake,      badgeClass: 'bg-blue-100 text-blue-700 border-blue-200',         chipClass: 'bg-blue-50 text-blue-700 border-blue-200' },
  email:      { label: 'Email',      icon: Mail,           badgeClass: 'bg-amber-100 text-amber-700 border-amber-200',      chipClass: 'bg-amber-50 text-amber-700 border-amber-200' },
  whatsapp:   { label: 'WhatsApp',   icon: MessageCircle,  badgeClass: 'bg-emerald-100 text-emerald-700 border-emerald-200', chipClass: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  default:    { label: 'Lainnya',    icon: History,        badgeClass: 'bg-slate-100 text-slate-600 border-slate-200',      chipClass: 'bg-slate-50 text-slate-600 border-slate-200' },
};

// Metadata tampilan per status update (label, warna badge/teks/dot timeline).
// Mengikuti pola warna getProjectStatus di lib/taskUtils.js.
export const UPDATE_STATUS_META = {
  baru:          { label: 'Baru',          badge: 'bg-slate-100 text-slate-700',     text: 'text-slate-700',   dot: 'bg-slate-400' },
  proses:        { label: 'Proses',        badge: 'bg-blue-100 text-blue-700',       text: 'text-blue-700',    dot: 'bg-blue-500' },
  tindak_lanjut: { label: 'Tindak Lanjut', badge: 'bg-amber-100 text-amber-700',     text: 'text-amber-700',   dot: 'bg-amber-500' },
  selesai:       { label: 'Selesai',       badge: 'bg-emerald-100 text-emerald-700', text: 'text-emerald-700', dot: 'bg-emerald-500' },
  default:       { label: 'Baru',          badge: 'bg-slate-100 text-slate-700',     text: 'text-slate-700',   dot: 'bg-slate-400' },
};

// Form kosong untuk membuat Update baru.
export const EMPTY_UPDATE_FORM = { title: "", category: "meeting", status: "baru", description: "" };

// Data awal (seeder frontend, opsional) untuk fitur Update.
